...
if($_REQUEST[planning_val]){
	$whereTxt .= " and mp_name = '".$_REQUEST[planning_val]."'";
}
if($_REQUEST[approval]){
	$whereTxt .= " and mp_open = '".$_REQUEST[approval]."'";
}
if($_REQUEST[event_subject]){
	$whereTxt .= " and mp_subject like '%".$_REQUEST[event_subject]."%'";
}
...
<div class="common_page common_none" style="width:1600px;"> 
	...
	<div class="cm_common_top" style="width:1600px;">
		<div class="commom_page_title">
			...
			<dl>
				<dt><a href="planning.list.php">이벤트 리스트</a></dt>
	...
	<div class="layout_fix"  style="width:1600px;">
		...
		<div class="cm_order_search" style="margin-top:20px;padding:15px">
			...
			<div class="detail">
				<form name="planning_search_form" id="planning_search_form" method="post" action="<?=$PHP_SELF?>">
					<span style="float:left;padding:10px 10px 0 10px">이벤트 제목</span>
					<input type="text" name="event_subject" id="event_subject" value="<?=$_REQUEST['event_subject']?>" class="input_date" style="background:none;width:100px;padding:0 5px"/>
					<span style="float:left;padding:10px 10px 0 10px">이벤트 게시 기간</span>
					...
					<input type="text" name="planning_name" id="planning_name" value="<?=$_REQUEST['planning_val']?>" class="input_date" style="background:none;width:100px;padding:0 5px"/>
					<span class="button_pack"><a href="#none" onclick="planning_search();" class="btn_md_color">검색</a></span>
					<span class="button_pack"><a href="event.create.php" onclick="" class="btn_md_black">이벤트 등록하기</a></span>
				</form>
			</div>
		</div>
		...
		<div class="cm_mypage_list list_posting">
			...
			<tbody>
				...
				$sql = "
					select * from odtEvent $whereTxt order by uid desc
				";
				...
				$filename = str_replace("/home/jooyon/CRM/uploads/eventimg/pc/", "", $v[mp_banner_image]);
				$filename2 = str_replace("/home/jooyon/CRM/uploads/eventimg/mobile/", "", $v[mp_banner_image_m]);
				
				$images_pc = ($v[mp_banner_image])?"<img src='../../uploads/eventimg/pc/".$filename."'>":"<img src='/skin/pc/images/noimage.png'>";
				$images_mobile= ($v[mp_banner_image])?"<img src='../../uploads/eventimg/mobile/".$filename2."'>":"<img src='/skin/pc/images/noimage.png'>";
				...
				<tr class="open_full" style="background:<?=$background?>">
					...
					<td>
						<span class="button_pack btn_close_conts"><a href="event.create.php?_mode=modify&_code=<?=$v[uid]?>" class="btn_sm_color" style="float:none;">수정</a></span>
						<span class="button_pack btn_close_conts"><a href="#none" onclick="event_del('<?=$v[uid]?>')" class="btn_sm_black" style="float:none;">삭제</a></span>
					</td>
				</tr>
				<? } ?>
			</tbody>
...
function planning_search(){
	$('#planning_search_form').attr("action","event.list.php");
